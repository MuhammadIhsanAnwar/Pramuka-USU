    ...
    <div class="rounded-4xl border border-slate-200 bg-white shadow-[0_30px_70px_rgba(15,23,42,0.08)] p-8 sm:p-10">
        <div class="grid gap-10 lg:grid-cols-[280px_auto] lg:items-start">
            <div class="space-y-5 rounded-[28px] border border-slate-200 bg-slate-50 p-6 text-center">
                    <div class="mx-auto flex h-40 w-28 items-center justify-center overflow-hidden rounded-[20px] bg-white shadow-sm sm:h-44 sm:w-32">
                        @if (filled($member->avatar_path))
                        <img src="{{ $member->avatar_url }}" alt="Foto {{ $member->name }}" class="h-full w-full object-cover" />
                        @else
                        <span class="text-4xl font-semibold text-slate-600">{{ strtoupper(substr($member->name, 0, 1)) }}</span>
                        @endif
                    </div>
            ...
            <div class="space-y-8">
                <div class="mb-6 rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <p class="mt-2 text-base font-semibold text-slate-900">{{ $member->nta ?? '-' }}</p>
                        </div>

                        <div class="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Jabatan</p>
                            <p class="mt-2 text-base font-semibold text-slate-900">{{ $member->jabatan ?? '-' }}</p>
                        </div>

                        <div class="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Satuan</p>
                            <p class="mt-2 text-base font-semibold text-slate-900">{{ $member->satuan ?? '-' }}</p>
                        </div>

                        <div class="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Golongan</p>
                            <p class="mt-2 text-base font-semibold text-slate-900">{{ $member->golongan ?? '-' }}</p>
                        </div>
                        ...
                        <div class="sm:col-span-2 rounded-3xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <p class="mt-2 text-base font-semibold text-slate-900">{{ $member->tingkatan ?? '-' }}</p>
                        </div>
                    </div>
                </div>
