...
>
    ...
    <div class="fi-page-header-main-ctn">
        ...
        @else
            ...
            @if (filled($headerActions) || $breadcrumbs || filled($heading) || filled($subheading))
                <x-filament-panels::header :actions="$headerActions" :actions-alignment="$headerActionsAlignment" :breadcrumbs="$breadcrumbs" :heading="$heading" :subheading="$subheading">
        ...
        <div class="fi-page-main" style="padding: 0.25rem 0 0;">
            @if ($subNavigation)
            ...
            <div class="fi-page-content" style="padding: 0;">
                <div class="fi-user-page-shell">
    ...
    @elseif ($this->isTableLoaded() && filled($this->defaultTableAction))
        <div wire:init="mountAction(@js($this->defaultTableAction), @if (filled($this->defaultTableActionArguments)) @js($this->defaultTableActionArguments) @else {} @endif, @js(['table' => true, 'recordKey' => $this->defaultTableActionRecord]))"></div>
    @endif

    @if (filled($this->defaultAction))
        <div wire:init="mountAction(@js($this->defaultAction) @if (filled($this->defaultActionArguments) || filled($this->defaultActionContext)) , @if (filled($this->defaultActionArguments)) @js($this->defaultActionArguments) @else {} @endif @endif @if (filled($this->defaultActionContext)) , @js($this->defaultActionContext) @endif)"></div>
    @endif
