...
@section('content')
    <section class="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <article class="surface-card overflow-hidden rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            ...
            @guest
                <div class="mb-6 rounded-3xl border border-emerald-100 bg-emerald-50 p-6 text-sm text-emerald-800">
                    <div class="font-semibold">Anda belum masuk</div>
                    <p class="mt-2">Silakan masuk untuk melanjutkan ke halaman "Lakukan Presensi". Setelah masuk, Anda akan dikembalikan ke halaman ini.</p>
                    <div class="mt-4">
                        <a href="{{ route('login.with_intended', ['intended' => url()->full()]) }}" class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-emerald-500">Masuk untuk Melakukan Presensi</a>
                    </div>
                </div>
            @else
